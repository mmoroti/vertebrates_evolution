...
```{r}
# Past data
#amphibia_dataset_trans_past <- decostand(amphibia_dataset_past[,2:4], "standardize")
#amphibia_dataset_trans_past <- cbind(Species = amphibia_dataset_past[,1],amphibia_dataset_trans_past)
#nrow(amphibia_dataset_trans_past)

model_anura_svl_past <- gls((trait_rate*1000) ~ OMI + Tol , data=amphibia_dataset_past, 
                       correlation = corPagel(1,anura_phy_pgls, form = ~Species))

summary(model_anura_svl_past)
plot(model_anura_svl_past)
plot(resid(model_anura_svl_past) ~ fitted(model_anura_svl_past))
qqnorm(resid(model_anura_svl_past))
qqline(resid(model_anura_svl_past))
...
model_squamata_svl_past <- gls((trait_rate*1000) ~ OMI + Tol, data=squamata_dataset, 
...
# False convergence model se não fizer a multiplicação do trait por mil 
model_squamata_svl <- gls(trait_rate * 1000 ~ OMI + Tol, data=squamata_dataset, 
                       correlation = corPagel(1,squamata_phy_pgls, form = ~Species))
summary(model_squamata_svl)

squamata_present <- plot(model_squamata_svl, main= "Present climate data")

plot_grid(squamata_past,
squamata_present, align = "h", labels = NULL)
```
...
model_birds_svl_past <- gls(trait_rate ~ OMI + Tol, data=birds_dataset_past, 
...
model_mammals_svl_past <- gls(trait_rate ~ OMI + Tol, data=mammals_dataset_past, 
                       correlation = corPagel(1,mammals_phy_pgls, form = ~Species))

summary(model_mammals_svl_past)
# R2(model_mammals_svl_past) erro
...
model_mammals_svl <- gls(trait_rate ~ OMI + Tol, data=mammals_dataset, 
                       correlation = corPagel(1,mammals_phy_pgls, form = ~Species))
summary(model_mammals_svl)
# R2(model_mammals_svl) erro
...
``` 
```{r}
models <- list(Frogs = model_anura_svl_past,
               Squamates = model_squamata_svl_past,
               Birds = model_birds_svl_past, 
               Mammals = model_mammals_svl_past)
