...
plot(resid(model_niche_reptilia) ~ fitted(model_niche_reptilia))
qqnorm(resid(model_niche_reptilia))
qqline(resid(model_niche_reptilia))
...
save(model_niche_reptilia,
     file = "03_analysis/model_niche_reptilia.RData")
...
df_trait_birds_bin <- df_trait_birds %>%
  mutate(across(
    .cols = c(Fos, Arb, Aqu, Ter), # TODO CHECAR
    .fns  = ~ as.integer(.x > 0.5)
  ))
```
...
plot(resid(model_niche_birds) ~ fitted(model_niche_birds))
qqnorm(resid(model_niche_birds))
qqline(resid(model_niche_birds))
...
save(model_niche_birds,
     file = "03_analysis/model_niche_birds.RData")

birds_diagnosis <- plot(model_niche_birds)
```
...
model_niche_mammals <- gls(taxa_fenotipica ~ OMI + Tol +
                           Arb + Fos + Ter + Aqu,
                         data = trait_mammals_normalize,
                    correlation = corPagel(1,mammals_phy,
                                           form = ~Scientific.Name))

plot(resid(model_niche_mammals) ~ fitted(model_niche_mammals))
qqnorm(resid(model_niche_mammals))
qqline(resid(model_niche_mammals))
...
save(model_niche_mammals,
     file = "03_analysis/model_niche_mammals.RData")
