...
write.tree(amphibia_phy_ultra, "BAMM/amphibia/amphibia_log/amphibia_fully_bamm.new")

# preparing data for setBAMMpriors
svl_amphibia <- log10(as.numeric(anura_traits_full$V2))
...
priors <- BAMMtools::setBAMMpriors(phy = anura_phy_full, traits = svl_amphibia, outfile = NULL) 

write.table(mass_mammals, "BAMM/amphibia/amphibia_log/amphibia_traits_fully_2.txt", quote = FALSE, col.names = FALSE,row.names = TRUE) 

generateControlFile(file = 'BAMM/amphibia/amphibia_log/controlfile_amphibia.txt', type = 'trait', params = list(
    treefile = 'amphibia_fully_bamm.new',
    traitfile='amphibia_traits_fully_2.txt',
...
svl_squamata <- log10(as.numeric(squamata_traits_full$V2))
...
# preparing traits
# Squamata
mass_mammals <- log10(as.numeric(mammals_traits_full$V2))
...
v1 <- vifcor(data_past, th=0.5) # identify collinear variables that should be excluded
v1
# data_past_select <- exclude(data_past,v1) VIF < 3

# Preparing dataset
data_present <- climate_dataset %>% select(BIO01, BIO04, BIO07, BIO11, BIO12, BIO15, BIO17)
v2 <- vifcor(data_present, th=0.7) # identify collinear variables that should be excluded
v2
data_present_select <- exclude(data_present,v2) # VIF < 3
```
...
# climate data
# present
dudi_present <- dudi.pca(data_present_select, scale = TRUE, scan = FALSE)
