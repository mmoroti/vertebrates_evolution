...
Check MCMC run convergence
```{r}
mcmcout <- read.table("BAMM/squamata/teste/mcmc_out.txt",sep=',', header=T)
tail(mcmcout)
plot(mcmcout$logLik ~ mcmcout$generation)
...
# Tue Apr 18 19:07:03 2023 ------------------------------
# BAMMTools
events_squamata <- read.table("BAMM/squamata/teste/event_data.txt", sep=",", header=T)
...
Check MCMC run convergence
```{r}
mcmcout <- read.table("BAMM/birds/birds/mcmc_out.txt",sep=',', header=T)
tail(mcmcout)
plot(mcmcout$logLik ~ mcmcout$generation)
