...
# summarize paleo-pgem bioclimate variables
bio01_past_mean <- bio01_past %>%
    mutate_at(c(names(bio01_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio01_past)/5001) %>%
    select(Long, Lat, T_mean)

bio04_past_mean <- bio04_past %>%
    mutate_at(c(names(bio04_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio04_past)/5001) %>%
    select(Long, Lat, T_mean)

bio07_past_mean <- bio07_past %>%
    mutate_at(c(names(bio07_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio07_past)/5001) %>%
    select(Long, Lat, T_mean)

bio11_past_mean <- bio11_past %>%
    mutate_at(c(names(bio11_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio11_past)/5001) %>%
    select(Long, Lat, T_mean)

bio15_past_mean <- bio15_past %>%
    mutate_at(c(names(bio15_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio15_past)/5001) %>%
    select(Long, Lat, T_mean)

bio17_past_mean <- bio17_past %>%
    mutate_at(c(names(bio17_past)), as.numeric) %>%
    mutate(T_mean = rowSums(bio17_past)/5001) %>%
    select(Long, Lat, T_mean)
```

```{r}
# convert to raster file
r <- raster(extent(min(bio01_past_mean$Long), max(bio01_past_mean$Long), min(bio01_past_mean$Lat), max(bio01_past_mean$Lat)), 
            resolution = 1, 
            crs = "+proj=longlat +datum=WGS84")

r <- rasterize(bio01_past_mean[,1:2], r, bio01_past_mean[,3])
plot(r)
```
...
effectiveSize(postburn$N_shifts) # must be at least 200
...
edata_squamata <- getEventData(squamata_phy_rooted, "BAMM/squamata/event_data.txt", burnin=0.1, type='trait')
