...
vert_assemblages <- data.table::fread(
  file.path(local_directory, "spatial_data",
  "Tetrapod_360.csv"), stringsAsFactors=T)

# Shapefile
grid_cells_sf <- sf::st_read(dsn = file.path(
    local_directory, "spatial_data","grid_110x110"),
...
grid_climate <- read_csv(
  file.path(
    local_directory, "spatial_data","climate_variables_grid.csv")
) %>%
...
TetraData <- data.table::fread(
  file.path(local_directory, "traits", "TetrapodTraits_1.0.0.csv"))
...
save(
  ...
  file = file.path(local_directory, "traits", "niche_df.RData")
)
```
