...
svl_size <- as.numeric(group_trait_filter$BodyLength_mm) # change trait if needed
names(svl_size) <- group_trait_filter$Scientific.Name
```

1.3. Save trait file

```{r}
write.table(svl_size, 
            file = file.path("BAMM", group, "/", paste0(group,"_body_traits.txt")),
            quote = FALSE, 
            #sep = "\t",  
            col.names = FALSE,
            row.names = TRUE)
...
generateControlFile(file = file.path("BAMM", group, "/", paste0("controlfile_", group, ".txt")),
                    type = 'trait', params = list(
                      treefile = 'tree.new',
                      traitfile= paste0(group, "_body_traits.txt"),
