...
position_niche <- as.numeric(group_trait_filter$OMI) # change trait if needed
names(position_niche) <- group_trait_filter$Scientific.Name
```

1.3. Save trait file

```{r}
write.table(position_niche, 
            file = file.path("BAMM", group, "/", 
                             paste0(group,"_niche_position.txt")),
            quote = FALSE, 
            #sep = "\t",  
            col.names = FALSE,
            row.names = TRUE)
