...
```{r}
trait <- "BodyLength_mm" # for amphibians and squamates
#trait <- "BodyMass_g" # for birds and mammals
```
...
```{r}
load(
  file.path(local_directory, "traits", "niche_df.RData"))

# add constant in zero values of Tol
min_pos <- min(
  niche_mammals_terrestrial_df$Tol[niche_mammals_terrestrial_df$Tol > 0])
constante <- min_pos * 0.1 # prop ao menor valor positivo

# Tol or OMI 
group_trait <- niche_mammals_terrestrial_df %>% # choice tetrapod group
...
```{r}
group <- "Aves" # definy group
directory <- file.path(
  local_directory, "BAMM", "body_size", group
) # choice folder of trait

# cria pasta "ratematrix" se ainda não existir
if (!dir.exists(file.path(directory, "ratematrix"))) {
  dir.create(file.path(directory, "ratematrix"), 
             recursive = TRUE)
}

for (i in 1:10) {
  
  arvore <- read.tree(file.path(directory,
                                paste0(group, "_Tree_", i, ".tre")))
  ...
  save(
    ...
    ratematrix_position,
    ratematrix_breadth,
    file = file.path(directory, "ratematrix",
